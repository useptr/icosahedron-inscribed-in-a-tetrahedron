...
ADSKTetrahedron::ADSKTetrahedron()
    : ADSKTetrahedron(AcGePoint3d::kOrigin, 1.0) {}
ADSKTetrahedron::ADSKTetrahedron(AcGePoint3d aptCenter, double adEdgeLength)
...
Acad::ErrorStatus ADSKTetrahedron::dwgOutFields(AcDbDwgFiler *pFiler) const {
  ...
  if ((es = pFiler->writeUInt32(ADSKTetrahedron::kCurrentVersionNumber)) !=
      Acad::eOk)
    return (es);
  //----- Output params
...
Adesk::Boolean ADSKTetrahedron::subWorldDraw(AcGiWorldDraw *mode) {
  ...
  try {
    ...
    // Массив индексов вершин для каждой грани тетраэдра, метод shell из
...
Acad::ErrorStatus
ADSKTetrahedron::subMoveGripPointsAt(const AcDbVoidPtrArray &gripAppData,
...
void ADSKTetrahedron::calculateVertices() noexcept {
  ...
  m_aVertices.append(AcGePoint3d(m_ptCenter.x - m_dEdgeLength / 2.0,
                                 m_ptCenter.y - sqrt3 / 6.0 * m_dEdgeLength,
                                 m_ptCenter.z - dHalfHeight));
  m_aVertices.append(AcGePoint3d(m_ptCenter.x + m_dEdgeLength / 2.0,
                                 m_ptCenter.y - sqrt3 / 6.0 * m_dEdgeLength,
                                 m_ptCenter.z - dHalfHeight));
  m_aVertices.append(AcGePoint3d(m_ptCenter.x,
                                 m_ptCenter.y + sqrt3 / 3.0 * m_dEdgeLength,
                                 m_ptCenter.z - dHalfHeight));
  m_aVertices.append(
      AcGePoint3d(m_ptCenter.x, m_ptCenter.y, m_ptCenter.z + dHalfHeight));
}
...
bool ADSKTetrahedron::runTests() const {
  ...
  if (!(std::abs(m_dEdgeLength - m_aVertices[0].distanceTo(m_aVertices[2])) <
        dTolerance))
    return false;
  if (!(std::abs(m_dEdgeLength - m_aVertices[0].distanceTo(m_aVertices[3])) <
        dTolerance))
    return false;

  if (!(std::abs(m_dEdgeLength - m_aVertices[3].distanceTo(m_aVertices[0])) <
        dTolerance))
    return false;
  if (!(std::abs(m_dEdgeLength - m_aVertices[3].distanceTo(m_aVertices[1])) <
        dTolerance))
    return false;
  if (!(std::abs(m_dEdgeLength - m_aVertices[3].distanceTo(m_aVertices[2])) <
        dTolerance))
    return false;
  return true;
}
