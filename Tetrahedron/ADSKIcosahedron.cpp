...
void ADSKIcosahedron::calculateVertices(ADSKTetrahedron & arTetrahedron)
{
	...
	auto addIntersections = [&](AcArray<AcGeLineSeg3d>& aCevians) { // добавляет в массив вершин икосаэдра точки пересечения цевианов
		for (int curI = 0; curI < 3; ++curI) { // находит точки пересечения 3 цевианов
			int nextI = (curI + 1) % 3; // перебираем по порядку все цевианы (0 и 1), (1 и 2), (2 и 0)
			AcGePoint3d resultPoint;
			aCevians[curI].intersectWith(aCevians[nextI], resultPoint); // находим точку пересечения цевиа
			m_aVertices.append(resultPoint); // точки пересечения цевианов это вершины икосаэдра
		}
		};
	...
	{ // bottom tetrahedron face
		const std::vector<std::pair<int, int>> aEdges{
			{1, 0}, {0, 2}, {2, 1}
		}; // Индексы точек рёбер описывающие одну из граней икосаэдра
		AcGePoint3dArray aDividedVertices = divideByGoldenRatio(aTetrahedronVertices, aEdges); // Делим ребра на золотое сечение. Получаем точки сметрично расположеные от каждой вершины тетраэдра
		AcArray<AcGeLineSeg3d> aCevians;
		aCevians.appendList(AcGeLineSeg3d(aTetrahedronVertices[0], aDividedVertices[2]), AcGeLineSeg3d(aTetrahedronVertices[1], aDividedVertices[1]), AcGeLineSeg3d(aTetrahedronVertices[2], aDividedVertices[0])); // получаем цевианы, соединяя вершины тетраэдра с точками полученными в aDividedVertices
	...
	{ // front tetrahedron face
		...
		AcGePoint3dArray aDividedVertices = divideByGoldenRatio(aTetrahedronVertices, aEdges);
		AcArray<AcGeLineSeg3d> aCevians;
		aCevians.appendList(AcGeLineSeg3d(aTetrahedronVertices[0], aDividedVertices[1]), AcGeLineSeg3d(aTetrahedronVertices[1], aDividedVertices[2]), AcGeLineSeg3d(aTetrahedronVertices[3], aDividedVertices[0]));
		addIntersections(aCevians);
	}
	{ // right-back tetrahedron face
		...
		AcGePoint3dArray aDividedVertices = divideByGoldenRatio(aTetrahedronVertices, aEdges);
		AcArray<AcGeLineSeg3d> aCevians;
		aCevians.appendList(AcGeLineSeg3d(aTetrahedronVertices[1], aDividedVertices[1]), AcGeLineSeg3d(aTetrahedronVertices[2], aDividedVertices[2]), AcGeLineSeg3d(aTetrahedronVertices[3], aDividedVertices[0]));
		addIntersections(aCevians);
	}
	{ // back-left tetrahedron face
		...
		AcGePoint3dArray aDividedVertices = divideByGoldenRatio(aTetrahedronVertices, aEdges);
		AcArray<AcGeLineSeg3d> aCevians;
		aCevians.appendList(AcGeLineSeg3d(aTetrahedronVertices[0], aDividedVertices[1]), AcGeLineSeg3d(aTetrahedronVertices[2], aDividedVertices[0]), AcGeLineSeg3d(aTetrahedronVertices[3], aDividedVertices[2]));
		addIntersections(aCevians);
	}
