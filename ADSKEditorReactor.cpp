...
//-----------------------------------------------------------------------------
ADSKEditorReactor::~ADSKEditorReactor() { Detach(); }
...
AcEditor *ADSKEditorReactor::Subject() const { return (acedEditor); }

bool ADSKEditorReactor::IsAttached() const { return (mbAutoInitAndRelease); }
void ADSKEditorReactor::commandWillStart(const ACHAR *aszCmdStr) {
  if (_tcscmp(aszCmdStr, _T("STRETCH")) && _tcscmp(aszCmdStr, _T("SCALE")) &&
      _tcscmp(aszCmdStr, _T("GRIP_STRETCH"))) {
    return;
  }
...
void ADSKEditorReactor::commandEnded(const ACHAR *aszCmdStr) {
  ...
  for (auto &objId : DocVars.docData().m_aChangedObjects) {
    ...
    auto *pTetrahedronWithInscribedIcosahedron =
        ADSKCustomPyramid::cast(pEntity);
