...
//-----------------------------------------------------------------------------
//----- The one and only document manager object. You can use the DocVars object
//to retrieve
...
Acad::ErrorStatus attachToAllTetrahedronWithInscribedIcosahedron(bool attach) {
  AcDbDatabase *pWorkingDatabase{
      acdbHostApplicationServices()->workingDatabase()};
  if (!pWorkingDatabase)
    return Acad::eNoDatabase;
  AcDbBlockTableRecordPointer pBlockTableRecord(
      pWorkingDatabase->currentSpaceId());
...
void detachAllTetrahedronWithInscribedIcosahedronReactors() {
  ...
  std::unique_ptr<AcApDocumentIterator> pIt(
      acDocManager->newAcApDocumentIterator());
  if (pIt == nullptr)
    return;
  // Store a pointer to the current document
  AcApDocument *pCurDoc = acDocManager->curDocument();
  // Iterate through all documents, make each document the current document and
  // call attachEmployeeReactorToAllEmployee(false)
...
CDocData::CDocData()
    : m_bScaleCommand(false), m_bDoRepainting(false),
      m_pDatabaseReactor(nullptr) {
